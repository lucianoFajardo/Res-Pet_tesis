...
class _EditPostViewState extends State<EditPostView> {
                ...
                child: ElevatedButton(
                    onPressed: () async {
                      try {
                        ...
                        final imagenSupabase = await supabase
                            .from("created_post_adoption_pet")
                            .select("photo_pet");
                        String imagenPathOriginal = imagenSupabase
                            .toString()
                            .substring(
                                126, imagenSupabase.toString().length - 2);
                        ...
                        await supabase.storage.from("imagenes").updateBinary(
                            imagenPath, imagenBytes!,
                            fileOptions:
                                FileOptions(contentType: "imagen/$imagenTipo"));
                        /*
                    await supabase
                      .from("created_post_adoption_pet")
  ...
  Future traerImagen() async {
    final imagenSupabase =
        await supabase.from("created_post_adoption_pet").select("photo_pet");
    setState(() {
      imagenURL = imagenSupabase
          .toString()
          .substring(13, imagenSupabase.toString().length - 2);
      print(imagenURL);
    });
  }
